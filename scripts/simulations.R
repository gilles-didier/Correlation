...
Long_hk <- function(arbre, mu)
{ 
nbN <- arbre$Nnode
nbT <- length(arbre$tip.label)
#
NouvLong <- fillLC(rep(0,nbT), arbre, mu=mu)$NL
NouvInc <- fillLC(rep(0,nbT), arbre, mu=mu)$NI
NouvGam <- fillLC(rep(0,nbT), arbre, mu=mu)$NG
ExpLong <- rep(NA,nbT+nbN)
for (j in (nbT+1):(nbT+nbN))
{
	IndFils <- which(arbre$edge[,1] == j)
	NumFils <- arbre$edge[IndFils,2]
	vchap <- NouvLong[NumFils]
	Inc <- NouvInc[NumFils]
	vini <- vchap - Inc
	Gamma <- NouvGam[NumFils]
	#
	#
	ExpLong[j] <- (vini[1] - vini[2] + Gamma[1] - Gamma[2]) / sqrt(sum(vchap))
}
ExpLong <- ExpLong[(nbT+1):(nbT+nbN)]
return(ExpLong)
}
